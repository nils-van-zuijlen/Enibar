CREATE TYPE panel_product AS (id INT, name VARCHAR, description VARCHAR, price DECIMAL(10,2));
CREATE VIEW panels_content AS
SELECT panels.name AS panel_name, array_agg((products.id, products.name, price_description.label, prices.value)::panel_product) AS panel_products, categories.id AS category_id, categories.name AS category_name, categories.alcoholic AS category_alcoholic, categories.color AS category_color FROM panels
